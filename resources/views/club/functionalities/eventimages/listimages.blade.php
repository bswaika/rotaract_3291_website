                    ...
                    @foreach($events as $key=>$event)
                        <tr scope="row">
                            @if(strtotime($event->start_date)-strtotime('now')<0)
                                ...
                                <td>
                                    <form class="form-horizontal" method="POST" action="{{url('/club/event/images/'.$event->id)}}" enctype="multipart/form-data">
                                        {{ csrf_field() }}
                                ...
                                <td><a href="{{url('/club/event/images/'.$event->id)}}" class="btn btn-primary">View Images</a></td>
                            @endif
                        </tr>
                    @endforeach
                @endif
