...
@section('content')
    <div class="container">
        ...
        <a href="/club/events/create" class="btn btn-primary">Add Event</a>
        <hr class="purple-hr">
        <div class="well">
                ...
                @if(count($events)>0)
                    @foreach($events as $key=>$event)
                        <tr scope="row">
                            ...
                            <td><a href="/club/events/{{$event->id}}/edit" class="btn btn-warning">Edit</a></td>
                            <td><form method="POST" action="{{url('/club/events/'.$event->id)}}"> {{ csrf_field()}} {{method_field('DELETE')}} <button type="submit" class="btn btn-danger">Delete</button> </form></td>
                        </tr>
                    @endforeach
                @endif
                </tbody>
