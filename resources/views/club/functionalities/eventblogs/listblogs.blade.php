...
@section('content')
    <div class="container">
        ...
        <a href="/club/event/blogs/create" class="btn btn-primary">Add Event Blog</a>
        <hr class="purple-hr">
        <div class="well">
                    ...
                    @foreach($events as $key=>$event)
                        <tr scope="row">
                            @if(strtotime($event->start_date)-strtotime('now')<0)
                                ...
                                <td><a href="/club/event/blogs/{{$event->blog->id}}/edit" class="btn btn-warning">Edit</a></td>
                                <td><form method="POST" action="{{url('/club/event/blogs/'.$event->blog->id)}}"> {{ csrf_field()}} {{method_field('DELETE')}} <button type="submit" class="btn btn-danger">Delete</button> </form></td>
                            @endif
                        </tr>
                    @endforeach
                @endif
